...
# Started hacking by jjwatt
# Here's how this is called a lot:
#    ./ci/run_envoy_docker.sh './ci/do_ci.sh dev'
# That means run_envoy_docker.sh and whatever else it calls runs first,
# and then (hopefully) inside the container, it runs './ci/do_ci.sh dev'
# There's a bunch of environment bullshit there, and
# there's a bunch of environment bullshit here.
# And, there's also setup_cache.sh and build_setup.sh, likely full of bullshit, too.
# They're sourced, so we can't predict the order of calls.
# They're conditionally sourced so something in them might set NO_BUILD_SETUP. idk.

# They also call it like this. There are ENVOY_ env vars. This changes build dir
#    ENVOY_DOCKER_BUILD_DIR=~/build ./ci/run_envoy_docker.sh './ci/do_ci.sh dev'

# This will supposedly build a release version of the Envoy binary
#    ./ci/run_envoy_docker.sh './ci/do_ci.sh release.server_only'

# For the final binary to be copied to a tar.gz,
...
# The ./ci/run_envoy_docker.sh './ci/do_ci.sh <TARGET>' targets are listed
# in ./ci/README.md

# TODO(jjwatt): I think everywhere in the docs that have a target of 'dev' is wrong
# There doesn't seem to be a dev check here. maybe it's bazel.dev?

# Run a CI build/test target, e.g. docs, asan.

set -e
# NOTE(jjwatt): Reformatted this to make it clearer. It's clearer that the OA
# wasn't a huge bash fan.
build_setup_args=""
if [[ "$1" == "format" \
	  || "$1" == "fix_proto_format" \
	  || "$1" == "check_proto_format" \
	  || "$1" == "docs" \
          || "$1" == "bazel.clang_tidy" \
	  || "$1" == "bazel.distribution" \
          || "$1" == "deps" \
	  || "$1" == "verify_examples" \
	  || "$1" == "publish" \
